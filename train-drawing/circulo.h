...
void desenhaCirculo(float x, float y, float raio, int num_linhas, bool preenchido)
{
    int i;
    GLfloat angulo;
    ...
    for(i = 1; i <= num_linhas; i++) // FOR usado para o calculo de cada ponto pertencente aos extremos das retas
    {
        ...
        glVertex2f(x + (-raio*sin(i * angulo)) , y + raio*cos(i * angulo));	// centro = (0,0), x = 0 e y = raio;
